...
RSpec.describe Parliament::Utils::Helpers::HistoryHelper do
  before(:each) do
  ...
  it 'is a module' do
    expect(Parliament::Utils::Helpers::HistoryHelper).to be_a(Module)
  end

  describe '#history' do
    it 'returns a history hash' do
      expect(Parliament::Utils::Helpers::HistoryHelper.history).to eq({
        start: nil,
        current: [],
        years: {}
      })
  ...
  describe '#add' do
    context 'when adding no data' do
      it 'does nothing' do
        ...
        subject.add(nil)
        expect(Parliament::Utils::Helpers::HistoryHelper.history).to eq(history)
      end
    end
    ...
    context 'when adding' do
      ...
      context 'valid data' do
        ...
        it 'adds to history' do
          Parliament::Utils::Helpers::HistoryHelper.add(data)
          expect(Parliament::Utils::Helpers::HistoryHelper.history).to eq({
    ...
    describe '#sort_history' do
      context 'with no data' do
        it 'does nothing' do
          Parliament::Utils::Helpers::HistoryHelper.add([])
          expect(Parliament::Utils::Helpers::HistoryHelper.history).to eq({
            start: nil,
            current: [],
            years: {}
      ...
      context 'with data' do
        ...
        it 'sorts first by end date' do
          expect(Parliament::Utils::Helpers::HistoryHelper.history[:years][10][0]).to eq(node3)
        end

        it 'sorts by start date if same end date' do
          expect(Parliament::Utils::Helpers::HistoryHelper.history[:years][10][1]).to eq(node1)
        end
      end
    end
  end
