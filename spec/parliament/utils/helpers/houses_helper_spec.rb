...
RSpec.describe Parliament::Utils::Helpers::HousesHelper, vcr: true do
  let(:house_request)   { Parliament::Utils::Helpers::FilterHelper.filter(Parliament::Utils::Helpers::ParliamentHelper.parliament_request.house_index, 'House') }
  ...
  context '.set_ids' do
    ...
    context '@commons_id and @lords_id already set' do
      it 'will return IDs without making a request' do
        ...
        expect(Parliament::Utils::Helpers::ParliamentHelper).to_not receive(:parliament_request)
      end
    end
