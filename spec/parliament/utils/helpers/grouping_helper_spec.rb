...
RSpec.describe Parliament::Utils::Helpers::GroupingHelper do

  let(:dummy_class)  { Class.new { include Parliament::Utils::Helpers::GroupingHelper }}
  let(:instance)     { dummy_class.new }

  describe '#group_data' do
    context 'with no data' do
      it 'returns a hash' do
        expect(instance.group_data([], [:grouping, :name])).to be_a(Hash)
      end
    end
    ...
    context 'with data' do
      ...
      it 'returns an array of grouped and ungrouped objects' do
        expect(instance.group_data(data, [:grouping, :name])).to eq({
          'test' => [node1, node3],
          'banana' => [node2],
          'UNKNOWN' => [node5, node4]
        })
